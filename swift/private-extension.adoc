...
===== 정리
* 하나의 구현체 내부에서 의미 있는 단위 코드가 길어져서 영역을 구분하기 위한 것이라면 extension보다 `//MARK:, //TODO:, //FIXME:` 방식이 나음. extension을 역할을 뚜렷하게 구별되는 경우 public, private 메소드를 구분하는 경우에 사용하는 것을 추천함. private extension은 모두 private로 감춰짐.
